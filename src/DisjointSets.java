...
//это структура данных, которая отслеживает набор элементов, разделенных на несколько непересекающихся (неперекрывающихся) подмножеств. Алгоритм поиска объединения - это алгоритм, который выполняет две полезные операции над такой структурой данных:
//		Найти: Определите, в каком подмножестве находится конкретный элемент. Это может быть использовано для определения того, находятся ли два элемента в одном и том же подмножестве.
